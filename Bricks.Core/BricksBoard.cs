...
namespace Bricks.Core
{
    public class BricksBoard : BaseBricksArray, IBoard
    {
        ...
        public override void InitializeArray(string shapeParam)
        {
            score = 0;
            level = 1;
            lines = 0;
            ...
            {
                next = GetDesiredShape(shapeParam);
            }
            presenter.UpdateScoreView(score, hiScore, lines, level, next);
            base.InitializeArray(shapeParam);
        }
        ...
        private void GameOver()
        {
            ...
            {
                {"Level", sLevel },{"Score",hiScore}
            };

            telemetry.TrackEvent("GameOver", null, metrics);
            presenter.GameOver();
        }
